...
// functions/sign-gathered-data/sign-gathered-data.ts
__export(exports, {
  handler: () => handler
});
...
var handler = async (event, context) => {
  ...
  try {
    ...
    const token = await new SignJWT({
      state,
      requiredData
    }).setSubject(payload.sub || "").setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt().setIssuer("https://auth0-progressive-profiling.netflify.app").setExpirationTime("60s").sign(Buffer.from(TOKEN_SIGNING || ""));
    return {
      statusCode: 200,
...
//# sourceMappingURL=sign-gathered-data.js.map
