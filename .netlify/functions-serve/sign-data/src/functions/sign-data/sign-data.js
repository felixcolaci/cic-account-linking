...
// functions/sign-data/sign-data.ts
__export(exports, {
  handler: () => handler
});
...
var handler = async (event, context) => {
  ...
  try {
    ...
    const token = await new SignJWT({
      ...
      user_id,
      action
    }).setSubject(payload.sub || "").setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt().setIssuer("https://cic-account-linking.netflify.app").setExpirationTime("60s").sign(Buffer.from(TOKEN_SIGNING || ""));
    return {
      statusCode: 200,
